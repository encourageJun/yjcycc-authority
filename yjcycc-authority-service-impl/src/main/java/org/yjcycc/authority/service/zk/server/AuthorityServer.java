package org.yjcycc.authority.service.zk.server;
...
import org.yjcycc.authority.service.api.*;
import org.yjcycc.authority.service.tools.prop.RmiProperties;
...
@Component
public class AuthorityServer extends AbstractZookeeperClientRegister implements Lifecycle, ApplicationContextAware ,InitializingBean {
	
	private static Logger logger = Logger.getLogger(AuthorityServer.class);
		...
		if(specifyIpPortIndex == null || "".equals(specifyIpPortIndex.trim())){
			...
			for(int index = 0; index < RmiProperties.MAX_NODES; index ++){
				try {
					...
					initUsingIpPort();
					isRun = true;
					logger.info("AuthorityServer "+index+" started ok!");
					return ;
			...
			logger.error("AuthorityServer 启动遇到问题，"+RmiProperties.MAX_NODES+"套配置没有一套有效的.....");
		}else{
